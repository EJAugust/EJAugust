...
This project is the culmination of my research in art, design, engineering, and mathematics. The result is a light, MVC-like front-end framework in which applications are made from reusable parts and is readily analyzed using mathematics.
# Concept
...
By giving every part type a [domain name ↗](https://datatracker.ietf.org/doc/html/rfc1034) (see [𝑓𝑖𝑔.3](#fig-3)) and expressing its value in some [base ↗](https://www.mathsisfun.com/numbers/bases.html) (see [𝑓𝑖𝑔.4](#fig-4)), we obtain a [URI ↗](https://datatracker.ietf.org/doc/html/rfc3986) for every value of every type (see [𝑓𝑖𝑔.5](#fig-5)).
## Implementation
Parts are implemented as javascript [`class ↗`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Classes) instances (see [𝑓𝑖𝑔.6](#fig-6)). Each part has presentation information which takes the place of a traditional model-view-controller paradigm with perhaps less overhead than one would expect. All values have the [`bigint` ↗](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt) type.
Type definitions are organized as source files in a folder starting at `domain-root/` with a structure matching the [domain name hierarchy ↗](https://www.hostnoc.com/dns-hierarchy-everything-you-need-to-know/). Types gain their names from the folder their source material is in and are defined by adding a new empty folder and putting zero or more source files into it. Certain file names are for special use like `.unit` for specifying a  base type. Presentation materials can also be placed in these folders. At deployment time, a node script recursively packs `domain-root` into a single script `sw.js` and a single document `index.html`.
## Figures
For any part $p_x$
- $`t_x=\text{type}(p_x)`$
- $`v_x=\text{value}(p_x)`$
...
|<sub><i>Parts </i></sub>$p_b$<sub><i> and </i></sub>$p_c$<sub><i> are base-10 numerals.</i></sub><br><br>$`t_b=t_c`$<br>$`v_b=...= 9`$<br>$`v_c=...= 4`$<br><sub><i>where </i></sub><br>&nbsp;&nbsp;$`k_b=k_c=10`$
...
|<sub><i>Given the following 74 url-friendly characters:</i></sub><br>\[`0`, ... `9`, `a` ... `z`, `A`, ... `Z`, `-`,  `_`, `.`, `~`, `?`, `#`, `&`, `/`, `=`, `!`, `$`, `*`\]<br><sub><i>the optimal base (for space efficiency) would be </i></sub>$b=74$<sub><i>.</i></sub>

|<span id=fig-5>Figure 5</span>
|-
|<sub><i>Joining host </i></sub>$h_x$<sub><i> to value </i></sub>$v_x$<sub><i> in base </i></sub>$b$<sub><i> obtains uri </i></sub>$u_x$<sub><i>.</i></sub>
